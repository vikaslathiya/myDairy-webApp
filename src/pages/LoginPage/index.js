...
import {
  GetAllAdmins,
  GetAllUsers,
} from "../../Redux/Actions/getUsers/getAllUsers";
...
const LoginPage = () => {
  ...
  const { data, error, loadingSignUp } = useSelector(
      ...
      {!loading && (
        <Grid container className={myStyle.container}>
          ...
          <Grid item lg={12} md={12} xs={12} className={myStyle.form}>
            {isLogIn && (
              <FormControl component="fieldset" className={myStyle.formControl}>
                <RadioGroup
                  row
                  aria-label="role"
                  name="role"
