...
export const updateUserAction = (user) => async (dispatch) => {
    ...
    if (response?.status === 200) {
        dispatch({type: CONSTANTS.UPDATE_USER_SUCCESS, payload: 'User details updated successfully'})
    } else {
        dispatch({type: CONSTANTS.UPDATE_USER_FAILED, payload: "Something went wrong!"})
    }
}
